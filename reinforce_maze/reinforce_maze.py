...
def update_theta(theta, pi, s_a_history):
    eta = 0.1 # 学習率
    gamma0 = 0.9 # Decay rate of return
    ...
    for t in range(T):
        state = s_a_history[t][0]
        action = s_a_history[t][1]
        gamma = gamma0 ** (T - t - 1)

        new_theta[state, action] += eta * gamma
        for a in range(n):
            new_theta[state, a] -= eta * gamma * pi[state, a]
...
print("Total of %d iteration" % count)
